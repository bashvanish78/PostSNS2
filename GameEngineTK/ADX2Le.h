...
class ADX2Le
{
	...
	static ADX2Le_Player m_playerBgm; // プレイヤー(BGM)
	static ADX2Le_Player m_playerSe; // プレイヤー(SE)
...
public:
	...
	// プレイヤーの取得関数
	static ADX2Le_Player* GetPlayerBgm();
	static ADX2Le_Player* GetPlayerSe();
	...
	// 指定キューの再生 
	static CriAtomExPlaybackId PlayBgm(CriAtomExCueId cue_id, float volume = 1.0f);
	static CriAtomExPlaybackId PlaySe(CriAtomExCueId cue_id, float volume = 1.0f);
	...
	// プレイヤーのポーズ状態の取得
	static bool IsPauseBgm();
	static bool IsPauseSe();
