...
SceneState* ViewScreenShotScene::pState = nullptr;
...
ViewScreenShotScene::ViewScreenShotScene()
{
}
...
SceneState * ViewScreenShotScene::GetInstance()
{
	if (pState == nullptr)
		pState = new ViewScreenShotScene();
	//pState = std::make_unique();
	return pState;
}

void ViewScreenShotScene::Execute(SceneManager* pManager, std::shared_ptr<DirectX::Keyboard> keyboard)
{
	// Device resources.
	m_window = _window;
...
void ViewScreenShotScene::SceneInitialize(std::shared_ptr<DirectX::Keyboard> keyboard)
{
	Initialize(m_window, m_outputWidth, m_outputHeight, keyboard);
}

void ViewScreenShotScene::SceneInitialize2(std::shared_ptr<DirectX::Keyboard> keyboard)
{
	//ポーズ中なら解除
...
void ViewScreenShotScene::SceneUpdate(SceneManager* pManager, std::shared_ptr<DirectX::Keyboard> keyboard)
{
	Update(m_timer, pManager, keyboard);
}

void ViewScreenShotScene::SceneRender()
{
	Render();
}

void ViewScreenShotScene::Dispose()
{
	if (pState != nullptr)
		delete pState;
}

void ViewScreenShotScene::SetPlayerDataOnResult(Player* player)
{
}
...
//初期化(ゲーム画面の横幅縦幅を引数)
void ViewScreenShotScene::Initialize(HWND& window, int& width, int& height, std::shared_ptr<DirectX::Keyboard> keyboard)
{
	//ウィンドウの設定
	m_window = window;
	...
	*/
	...
	m_SSSprite.LoadTextureSetAll(L"Resources/ScreenShots0.bmp", Vector2(800, 600), Vector2(0.75f, 0.75f), Vector2(800 / 2.f, 600 / 2.f));

}

// Executes the basic game loop.
//UpdateとRender呼び出し
void ViewScreenShotScene::Tick(SceneManager* pManager, std::shared_ptr<DirectX::Keyboard> keyboard)
{
	m_timer.Tick([&]()
	{
		Update(m_timer, pManager, keyboard);
...
// Updates the world.
void ViewScreenShotScene::Update(DX::StepTimer const& timer, SceneManager* pManager, std::shared_ptr<DirectX::Keyboard> keyboard)
{
...
// Draws the scene.
void ViewScreenShotScene::Render()
{
...
void ViewScreenShotScene::Clear()
{
	// Clear the views.
...
void ViewScreenShotScene::Present()
{
...
// Message handlers
void ViewScreenShotScene::OnActivated()
{
	// TODO: Game is becoming active window.
}

void ViewScreenShotScene::OnDeactivated()
{
	// TODO: Game is becoming background window.
}

void ViewScreenShotScene::OnSuspending()
{
	// TODO: Game is being power-suspended (or minimized).
}

void ViewScreenShotScene::OnResuming()
{
...
void ViewScreenShotScene::OnWindowSizeChanged(int width, int height)
{
	m_outputWidth = max(width, 1);
	m_outputHeight = max(height, 1);
...
// Properties
void ViewScreenShotScene::GetDefaultSize(int& width, int& height) const
{
...
void ViewScreenShotScene::CreateDevice()
{
...
void ViewScreenShotScene::CreateResources()
{
...
void ViewScreenShotScene::OnDeviceLost()
{
